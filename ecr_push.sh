...
docker buildx build --platform linux/amd64 -t sb-fsts:latest --load .

# 2. 로컬에 로드된 이미지를 ECR 레포지토리로 태그 변경
docker tag sb-fsts:latest 196441063343.dkr.ecr.ap-northeast-2.amazonaws.com/sb-fsts-ecr:$ECR_IMAGE_TAG
